...
>
>  2. Using the function `future::plan(multisession(workers = XX))`, declare a "*plan*" of parallel computations on your computer (take care to **always leave at least one core available** to handle other processes).
>
...
The `pbapply` package allows to get a nice progress bar for parallelized `*apply` functions execution.
